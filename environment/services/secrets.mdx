...
All secrets are by default encrypted and stored in your connected cloud account. Sensitive refers to how secrets are
handled during build time or runtime in Docker and Kubernetes environments.

For example: In Docker builds, by default the secrets are passed using
[build arguments](https://docs.docker.com/build/building/variables/#build-arguments), while sensitive items are passed
using [build secrets](https://docs.docker.com/build/building/secrets/).
