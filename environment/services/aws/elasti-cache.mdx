---
title: Elasticache Redis, Valkey & Memcache
description: Provision AWS Elastic Cache clusters - Redis/Valkey/Memcache
---

Your service(s) may want to store/access data in a Redis/Valkey/Memcache instance. This guide will show you steps to
create and access Amazon ElastiCache cluster that offers fully managed Redis/Valkey/Memcache databases.
...
3. `engine` - Provide `redis`, `valkey` or `memcache`. Default: ``. If you leave this out, we will skip creating the
   cluster.
4. `version` - Pick a version for your redis, valkey or memcache database. Ensure Elastic Cache supports it, by
   referring to [AWS docs](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/engine-versions.html). For Valkey,
   see the [supported Valkey versions](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/supported-engine-versions-valkey.html).
...
7. `parameters` - Optional list of cache parameter group settings to apply to the cluster. Each entry is an object with
   the following keys:
   - `name` - Name of the parameter (e.g., `maxmemory-policy`). Must be a valid parameter for the chosen `engine` and
     `version`. Refer to the
     [Redis/Valkey parameter docs](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/ParameterGroups.Redis.html)
     or
     [Memcached parameter docs](https://docs.aws.amazon.com/AmazonElastiCache/latest/mem-ug/ParameterGroups.Memcached.html)
     for available names.
   - `value` - Value to set for the parameter. Always specified as a string.

   When `parameters` is provided, LocalOps creates a dedicated cache parameter group for the cluster and attaches it.
   Adding, updating or removing entries on subsequent deployments updates the parameter group in-place.
8. `exports` - Set of key value pairs. Keys are the ENVIRONMENT VARS we will pass to your code / containers. Values are
...
Ping us on slack / write to us at support@localops.co / [book a call](https://go.localops.co/meet-engineer) with our team
