...
For your service, if you need one or more SNS topics, you can add `ops.json` in the root directory of Github repo you've connected for the service. And add SNS topics as dependency, like below.
...
1. `id` - Alpha numeric string. Must be unique amongst the Topics you've declared above. Changing this string will replace the original topic with new one.
2. `prefix` - Alpha numeric string. Will be used as a prefix in the name of your Topic. Changing this string will replace the original topic with new topic.
3. `exports` - Set of key value pairs. Keys are the ENVIRONMENT VARS we will pass to your code / containers. Values are the properties of the Topic provisioned. See below for available properties. 

<Warning>
Changing `id` or `prefix` string will delete & replace the original topic with new one.
</Warning>
...
- When the service is deleted, the provisioned resources (topics, here) are deleted from the cloud account immediately & automatically. 
