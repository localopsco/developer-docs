---
title: S3
description: Provision S3 buckets for object storage
---
...
connected for the service. And add S3 buckets as dependency, like below.
...
1. `id` - Alpha numeric string. Must be unique amongst the buckets you've declared above. Changing this string will
   replace the original bucket with new one.
2. `prefix` - Alpha numeric string. Will be used as a prefix in name your bucket. Changing this string will replace the
   original bucket with new bucket.
