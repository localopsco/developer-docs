...
- `image` - Image name with tag/version. Eg., `nginx:1.29`. This is optional. If the `image` is not provided, we use the same image that was used to run your original service, which was built with the latest commit in your branch/PR.
- `cmd` - start command. Used on the `image` you provide in this config or the image LocalOps built using latest commit
   in your branch/PR.
- `env` - Pass any environment variable necessary to run this image. In addition, when you don't provide an `image` in this config, the image LocalOps built using latest commit in the PR will be used and all the secrets you added for this service in LocalOps console UI will also be injected into the job.

#### Run the job once accross all the pods

Let's say you want to run migration for your DB or seed your data, By default all the job mentioned in the init container will be run once per every copy/pod that is created. To control the behaviour for your job to run once across all pods, you can set the `once` parameter to `true`.

<Note>
    When `once` is set, the `cmd` must be a fully qualified command not just arguments. By default kubernetes or helm doesn't support this, we override your entrypoint(if exists) in your Dockerfile with a custom entrypoint script to get the desired behaviour.
</Note>
...
Checks will be done on each container of your service. Containers are marked as healthy if their corresponding health checks pass for at least once or for `success_threshold` times or more.

You can define how often checks have to be carried out using `interval`. And you can define `delay` to tell us how long to wait to start making those checks, during the first time.

If your container fails these checks beyond `failure_threshold`, the corresponding container will be restarted automatically. 
