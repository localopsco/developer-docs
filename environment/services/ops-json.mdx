...
You can declare these dependencies once in `ops.json`. LocalOps will provision these resources and ensure they are active & enabled for your service to use.
...
You can define the environment variable keys and specify the properties of the cloud resource to fill it up. So in above example, LocalOps will form an environment variable by the name `ATTACHMENTS_BUCKET_ARN` and pass ARN of the new S3 bucket to it. So that in your code, you can use it like:
...
Resources are read from `ops.json` when the [service](/environment/services/create) is being [deployed](/environment/services/deploy). 
...
Your code can access these resources using the environment variables defined in `ops.json`. For example, you can add files/objects to the S3 bucket you defined in `ops.json` (see above), like:
...
For the resources defined in `ops.json`, LocalOps automatically generates necessary IAM permissions and add them as policies in the pre-conigured IAM role. Your code can just access the resources defined in `ops.json` by name and all calls made will be authenticated calls by default.

For example, for the S3 bucket defined in `ops.json`, LocalOps automatically adds the following necessary IAM permissions in the pre-configured IAM role. 
...
1. **Amazon S3 buckets** - for static object storage (files, pictures, etc.,). [Learn more](/environment/services/aws/s3)
2. **Amazon RDS Instances** - for hosting Postgres, MySQL databases. [Learn more](/environment/services/aws/rds)
3. **Amazon Elastic cache clusters** - for caching, queues, pub/sub.  [Learn more](/environment/services/aws/elasti-cache)
...
- **Monitoring**: Cloudwatch monitoring is turned on by default for applicable cloud resources. These metrics and logs can be seen inside out [built-in monitoring dashboard](/environment/monitoring/metrics).
...
- schedule: Specify a spechedule using the same powerful crontab syntax. [Refer here](https://crontab.guru/) on how to define this.
...
LocalOps spins up an [implicit cron service](https://hub.docker.com/r/localopsroot/opscron) within your environment, to make the API calls to your service using its private Internal host name, as per the above schedule. 
