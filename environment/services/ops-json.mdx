...
Applicable and run only for a [web](/environment/services/web) service.

Say you want to run a db migration before a service is started. Just add the job inside `init` key like below:
...
3. `image` - Image name with tag/version. Eg., `nginx:1.29`. This is optional. If the `image` is not provided, we use
   the same image that was used to run your original service, which was built with the latest commit in your branch/PR.
...
3. If this is running as part of a preview service, preview service `previews.depdendencies` > `exports` variables are added too.
...
4. `cmd` - start command. Used on the `image` you provide in this config or the image LocalOps built using latest commit in your PR. Refer to [run command in service](/environment/services/create#run-command) for more details
5. `env` - Pass any environment variable necessary to run this image. In addition, when you don't provide an `image` in
   this config, the image LocalOps built using latest commit in the PR will be used and all the secrets you added for
