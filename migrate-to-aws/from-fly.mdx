...
<div style={{ position: "relative", paddingBottom: "66.58%", height: 0 }}>
  <iframe
    src="https://www.loom.com/embed/0bf8ad4510bf401aa429924ad2364a51"
    frameBorder="0"
    allowFullScreen
    style={{
      position: "absolute",
...
<Tip>
  **White-glove migration**: Our engineers will migrate your Fly.io app to AWS.
  [Schedule a migration call](https://go.localops.co/fly) and we'll handle
  everything for you.
</Tip>
...
<Info>
  Need help? [Schedule a migration call](https://go.localops.co/fly) and our
  engineers will assist you through the entire process.
</Info>
...
<Steps>
  <Step title="Create LocalOps account" icon="user">
    [Sign up for LocalOps](https://console.localops.co/signup) if you haven't
    already.
  </Step>
  <Step title="Connect AWS account" icon="cloud">
    Follow the [AWS connection guide](/accounts/aws) to connect your AWS
    account.
  </Step>
  <Step title="Create environment" icon="container-storage">
    Create a new environment (e.g., production) in your preferred AWS region.
    See [Create new environment](/environment/create).
  </Step>
  <Step title="Create service" icon="server">
    Create a new service and connect your GitHub repository. See [Create new
...
<Warning>
  For large databases, the backup and restore process may take significant time.
  Plan for a maintenance window if you need zero data loss during migration.
</Warning>
...
<Tip>
  If you used `ops.json` to create RDS, the password is stored in AWS Secrets
  Manager. Use the AWS SDK to retrieve it using the `DATABASE_PASSWORD_ARN`
...
<Info>
  Learn more about [logs](/environment/monitoring/logs),
  [metrics](/environment/monitoring/metrics), and
...
The [LocalOps CLI](/cli/install-macos) covers the two `flyctl` commands you probably ran most often:

| Fly CLI           | LocalOps CLI                                        |
| ----------------- | --------------------------------------------------- |
| `fly logs`        | [`ops logs`](/cli/usage#stream-logs-from-a-service) |
| `fly ssh console` | [`ops connect`](/cli/usage#shell-into-a-service)    |
| `fly proxy`       | [`ops proxy`](/cli/usage#forward-a-service-port-to-localhost) |

```sh
ops logs --env production --svc web
ops connect --env production --svc web
```

Both take the environment and service names you see in your dashboard, so there is no machine id to look up first. When
a service runs several pods, `ops logs` reads all of them and prefixes each line with the pod it came from.

## Get help with your migration

<Tip>
  **White-glove migration**: Don't want to do this yourself? Our engineers will
  migrate your entire Fly.io setup to AWS—including database migration,
  environment variables, and custom domains. [Schedule a migration call
