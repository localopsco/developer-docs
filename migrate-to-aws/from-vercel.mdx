...
<Tip>
  **White-glove migration**: Our engineers will migrate your Vercel app to AWS. [Schedule a migration
  call](https://go.localops.co/vercel) and we'll handle everything for you.
</Tip>
...
<Info>
  Need help? [Schedule a migration call](https://go.localops.co/vercel) and our engineers will assist you through the
  entire process.
</Info>
...
Once it is up, the [LocalOps CLI](/cli/install-macos) gives you two useful checks from your terminal - streaming the
service's logs, and opening a SQL shell on the migrated database to confirm the data landed:

```sh
ops logs --env production --svc web
ops connect --env production --svc web --dep
```
...
<Tip>
  **White-glove migration**: Don't want to do this yourself? Our engineers will migrate your entire Vercel setup to
  ...
  now](https://go.localops.co/vercel).
</Tip>
