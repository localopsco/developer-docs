...
If you would rather watch the rollout from your terminal, the way you did with `heroku logs --tail`, install the
[LocalOps CLI](/cli/install-macos) and stream the service directly:
...
## Heroku CLI equivalents

The [LocalOps CLI](/cli/install-macos) covers the two commands you probably ran most often:

| Heroku CLI           | LocalOps CLI                                        |
| -------------------- | --------------------------------------------------- |
| `heroku logs --tail` | [`ops logs`](/cli/usage#stream-logs-from-a-service) |
| `heroku run bash`    | [`ops connect`](/cli/usage#shell-into-a-service)    |
| `heroku config`      | `ops connect --env <env> --svc <svc> -- printenv`   |
| `heroku pg:psql`     | [`ops connect --dep`](/cli/usage#open-a-psql-shell) |
...
Unlike `heroku run`, `ops connect` opens a shell on a pod that is already serving traffic rather than starting a fresh
one-off dyno. Anything you change there is lost when the pod is replaced, so use it to inspect and debug. Run one-off
tasks as a [job service](/environment/services/job) instead.
