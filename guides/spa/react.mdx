...
This block serves the `index.html` from the files we built using Vite, stored in the `/usr/share/nginx/html` folder. We
...
`example.com/api`, the nginx server acts as a reverse proxy server and forwards the request to the backend server. You
can use reverse proxying to navigate the traffic that is inbound to nginx. The reverse proxy server also acts as a mask,
hiding the real backend from the outside world.
