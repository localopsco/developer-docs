...
After you connect your cloud account, you must create a new environment for each of your deployment environments -
...
LocalOps provisions environments using standard in-house provisioning templates and scripts such that you can create
any number of environments on any cloud account (AWS, Google Cloud or Azure) and any region.
...
environment running. You can connect your cloud account via keyless, role-based access method from Connections
section.

### Exclusive Infrastructure

Each environment get its own exclusive set of kubernetes cluster, compute servers and other resources listed above.
...
Within each environment, LocalOps turns ON all security best practices suggested by the respective cloud provider.
