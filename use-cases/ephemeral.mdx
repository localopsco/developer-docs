...
When a new pull request is raised in the same github repo, from source branch `f-feature-new` targeting to merge into
`main` branch,
...
2. a new comment is posted on the PR page in Github containing the pubilc URL fo the service and a link to see
   deployment logs.
...
You may use 3rd party services like Slack, Google Apps, and others for Integration, Authentication, Notificaitons etc.,
