...
### How it works
Create a [new environment](/environment/create) if one doesn't exist. This will create a VPC and EKS cluster to host the ephemeral services.

1. Create a [new service](/environment/services/create) (say, `backend`) in the environment based on a specific Github repo and branch name (say, `main` branch)
