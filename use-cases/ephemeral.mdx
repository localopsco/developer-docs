...
Checkout [cron docs here](/environment/service/ops-json#configuring-cron-service) for more info. 
...
2. If the 3rd party allows you to do #1 via their API, you can do so in your application/service boot-up logic, when the preview service comes up. LocalOps passes in `LOC_PREVIEW=true` in all preview services as in-built environment variable. 
3. When there is a possibility to share the same 3rd party integration app/id and create a new redirect URL in the app's settings via their API, you can do the same, as a simplified case of #2 above. LocalOps passes `LOC_HOST` environment variable with the latest public url of the service.
