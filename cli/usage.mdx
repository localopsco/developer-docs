...
<Note>
  Only Postgres is supported today. A MySQL or Redis dependency is reported
  rather than opened. `--db-image` overrides the client image if you need a
  specific `psql` version.
</Note>
...
```sh
ops proxy --env staging --svc api --dep
```
...
```sh
ops proxy --env staging --svc api --dep --local-port 15432
```
...
The `kubectl` recipes below do the same work by hand. Reach for them when you need something `--dep` does not cover
yet - a MySQL shell, or a dependency that exports discrete fields rather than `$dsn`.
