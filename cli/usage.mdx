...
k9s will automatically use the current Kubernetes context set by `ops update-kubeconfig`. From the k9s interface you can:
...
To get a shell inside a running pod, first identify the pod name:

```sh
kubectl get pods --all-namespaces
```

Find your service's pod from the list, then exec into it:
