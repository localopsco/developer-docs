...
LocalOps CLI `ops` can be used to read logs from your services and to get shell access to them and to the underlying
Kubernetes cluster of an environment.
...
For the two things you reach for most often - reading logs and opening a shell - `ops logs` and `ops connect` talk to
your environment directly. You need neither `kubectl` nor the pod and namespace names:

1. Login to your LocalOps account with `ops login`
2. Run `ops logs` or `ops connect` against an environment and service by name
...
<Note>
  We are working on shortcut commands in the `ops` CLI to do all of this in a
  single step - one command to open a SQL shell on your environment's database,
  and one to proxy it to `localhost` for your favourite GUI client. Until those
