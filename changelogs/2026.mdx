...
## New options in ops.json: bigger uploads, fewer surprise errors

We have raised the platform limits that teams most often hit when they onboard, so a normal app should now run
without touching any of this. Four new `ops.json` options let you set your own limits when your service needs more.

### `request_body_size`: bigger uploads work out of the box

Every environment's ingress used to reject any request body over 1 MB, so an ordinary file upload came back as a
`413 Request Entity Too Large` before it ever reached your container. The platform now accepts **30 MB** by default, in
line with what other hosting platforms allow.

If your service takes bigger uploads - attachments, media, bulk imports - raise the limit yourself with
...
Anything up to 1 GB is allowed. [Read the docs](/environment/services/ops-json#request-body-size).

### `proxy_buffer_size` and `proxy_buffers`: fix `502`s caused by large response headers

Apps that hand back long headers - SSO redirects, big session cookies, fat JWTs - could fail with a
`502 Bad Gateway`, because the ingress reserved only a small buffer for a response's headers. That buffer is now
bigger by default, and you can size it yourself when your app needs more:
...
`proxy_buffer_size` is the size of each buffer in KB and is the one that fixes the `502`. `proxy_buffers` is how many
of them the ingress uses while streaming a response, and is usually fine left alone.
[Read the docs](/environment/services/ops-json#response-buffers).

### `send_timeout`, `read_timeout` and `connection_timeout`: slow requests get more room

The ingress waits longer before giving up on your service. Sending and reading now allow **120 seconds** each, up from
60, and connecting to your container is capped at 10 seconds so a genuinely dead container fails fast instead of
hanging. Slow reports, large downloads and chatty websocket handshakes stop getting cut off for no obvious reason.

You can still set `send_timeout`, `read_timeout` and `connection_timeout` per service, up to an hour.
[Read the docs](/environment/services/ops-json#request-timeouts).

### `deploy_timeout`: give slow-starting services longer to deploy

A deployment waits 5 minutes for your new version to become ready, then fails. That is too short for services with
large images or heavy startup work, so you can now extend it:
...
Anything from 1 to 20 minutes is allowed. Every deployment logs the wait it is using, so you can confirm your value was
picked up. Services on Windows node groups already get 15 minutes by default.
[Read the docs](/environment/services/ops-json#deploy-timeout).
...
## CLI v3.1.0 - Connect with pg shell or set up port forwarding

We added two new commands that let you connect to the RDS Postgres shell directly from your dev laptop. All without signing into bastion servers or any other step.

```sh
ops connect --env prod --svc web --dep appdb
```
Above command opens a postgres shell so you can start querying the database.

```sh
ops proxy --env prod --svc web --dep appdb
```
This command sets up port-forwarding so you can use GUI tools to connect to the same postgres database with host set to localhost and port set to 5432.

All of this will soon work for redis, mysql and sql server too! Stay tuned!

#### New CLI menu

![Build context](./images/2026/pgshell.png)

You can find these commands to copy-paste, from the new "CLI" tab in Environment/Service dashboard.

#### Install/upgrade to v3.1.0

Both commands ship in CLI **v3.1.0**. If you're new to the CLI, install it with Homebrew and sign in:

```sh
brew install localopsco/tap/ops
ops login
```

Already have `ops`? Upgrade in place:

```sh
brew update && brew upgrade ops
```

Both commands take more options than shown here, like `-i`. See the [CLI docs](/cli/usage) for the full list, and for
...
## CLI v3.0.4 — Shell into a service and tail its logs with one command

Getting a shell inside a running service used to mean pointing your kubeconfig at the environment, listing pods in
`app-services` and picking the right one before `kubectl exec`. Two new CLI commands cut that down to a single line
each.

```sh
ops connect --env "ProdUS" --svc web
```

opens a shell in your service, so you can do ad-hoc script runs or drop into your Django / Rails / other application
shell.

```sh
ops logs --env "ProdUS" --svc web
```

tails logs in real time. You don't have to open Grafana just to watch what a service is printing.

#### New shell tab

![Build context](./images/2026/CLI-release-screenshot.png)

You can find these commands to copy-paste, from the new "Shell" tab in Service dashboard.

#### Install/upgrade to v3.0.4

Both commands ship in CLI **v3.0.4**. If you're new to the CLI, install it with Homebrew and sign in:
...
## Cluster your BEAM nodes across regions

Elixir services running in separate regional environments can now form a single cluster. Keep `beam_cluster_id`
identical everywhere, and give each environment its own record in a private DNS zone that every regional VPC can
resolve:
...
LocalOps creates that record and keeps it pointed at the environment's pods as they scale, restart and get
rescheduled. Add one libcluster topology per region — the local nodes via `LOPS_BEAM_CLUSTER_DNS`, each peer region by
its `beam_dns_name` — and the mesh forms. The regional VPCs still need routable connectivity between them.
[Read the docs](/environment/services/beam-clustering#cluster-across-regions).
